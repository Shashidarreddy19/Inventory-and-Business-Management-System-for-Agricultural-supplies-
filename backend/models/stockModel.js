...
class Stock {
    ...
    static async delete(stockId, userId) {
        try {
            // First check if the stock item exists and belongs to the user
            const stockItems = await db.executeQuery(
            ...
            // Check if the stock item is referenced in credit_products
            const creditReferences = await db.executeQuery(
                'SELECT COUNT(*) as count FROM credit_products WHERE product_id = ?',
                [stockId]
            );

            if (creditReferences[0].count > 0) {
                return {
                    success: false,
                    message: 'This product cannot be deleted because it is used in credit transactions.'
                };
            }

            // Check if the stock item is referenced in sales
            const salesRecords = await db.executeQuery(
                'SELECT COUNT(*) as count FROM sale_items WHERE product_id = ?',
                [stockId]
            );

            if (salesRecords[0].count > 0) {
                return {
                    success: false,
                    message: 'This product cannot be deleted because it has sales records.'
                };
            }
