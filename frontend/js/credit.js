...
const API_BASE_URL = 'http://localhost:3001/api'; // Make sure this matches your backend URL
...
async function fetchCreditDetails(creditId) {
    ...
    const response = await fetch(`/api/credit/${creditId}?userId=${userId}`);
...
async function openPaymentHistory(creditId) {
    try {
        const userId = getUserId();
        const response = await fetch(`/api/credit/${creditId}/payments?userId=${userId}`);
...
async function toggleArchiveCredit(creditId) {
    try {
        ...
        const userId = getUserId();
        const response = await fetch(`/api/credit/${creditId}/${action}?userId=${userId}`, {
            method: 'PUT',
    ...
    return new Promise((resolve, reject) => {
        debounceTimers.set(creditId, setTimeout(async () => {
            try {
                ...
                const userId = getUserId();
                const requestPromise = fetch(`/api/credit/${creditId}?userId=${userId}`)
                    .then(async response => {
                        if (!response.ok) {
...
async function applyInterest(creditId, userId, days) {
    try {
        ...
        const response = await fetch(`/api/credit/${creditId}/interest`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
...
async function archiveCredit(creditId) {
    ...
    try {
        const userId = getUserId();
        const response = await fetch(`/api/credit/${creditId}/archive?userId=${userId}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' }
        });
...
async function unarchiveCredit(creditId) {
    ...
    try {
        const userId = getUserId();
        const response = await fetch(`/api/credit/${creditId}/unarchive?userId=${userId}`, {
            method: 'PUT'
        });
...
async function fetchAllPaymentHistory(credits) {
    ...
    try {
        for (const credit of credits) {
            const response = await fetch(`/api/credit/${credit.credit_id}/payments?userId=${userId}`);
            if (response.ok) {
                const payments = await response.json();
